...
class ApiClient:
    ...
    def _generate_headers(self, method: str, path: str) -> dict:
        ha1 = hashlib.md5(f"{self._username}:{self._realm}:{self._password}".encode()).hexdigest()
    ...
    async def request(
        ...
        async with self._lock:
            ...
            async with self._session.request(method, url, json=json_data, headers=headers) as response:
                if response.status == 401:
                    self._extract_digest_params(response.headers)
                    headers = self._generate_headers(method=method, path=path)

                    async with self._session.request(method, url, json=json_data, headers=headers) as refreshed_response:
